...
- `setting`: [Setting](#Setting) 매장 설정 정보
...
- `showOrderMenuImage`: 홀 관리 주문 메뉴 이미지 출력 여부
...
- `posTableActivity`: [PosTableActivity](#PosTableActivity) POS 테이블 액티비티
- `type`: [OrderPaymentType](#OrderPaymentType) 주문 결제 타입
...
- `moveTable()`: 테이블을 이동한다.
- `issueCashReceipt()`: 현금 영수증을 발급한다.
- `isPendingType()`: 주문 결제의 타입이 승인 대기 주문의 결제인지 여부를 반환한다.
- `isOrderedType()`: 주문 결제의 타입이 승인 완료 주문의 결제인지 여부를 반환한다.
- `isPureCash()`: 주문 결제의 결제 수단이 순수 현금 결제(현금영수증 X)인지 여부를 반환한다.
...
- `PENDING_ORDER`: 승인 대기 중인 주문의 결제
- `ORDERED_ORDER`: 승인 완료 주문의 결제
...
### FailedIssueCashReceiptException
...
- `static copy()`: 새로운 프린트 번호로 레시피를 복사한다.
...
- `hasOrderedOrder()`: 활성 POS 테이블 액티비티에 주문 상태의 주문이 있는지 여부를 반환한다.
- `hasPendingOrder()`: 활성 POS 테이블 액티비티에 승인 대기 상태의 주문이 있는지 여부를 반환한다.
- `getTableOrderType()`: 활성 POS 테이블 액티비티의 주문 타입을 반환한다.
...
- `getOrderedOrPendingOrders()`: 활성 POS 테이블 액티비티에 승인 대기 상태의 주문이 있다면
  승인 대기 상태의 주문 목록을 반환하고 아니라면 주문 상태의 주문 목록을 반환한다.
- `getOrderedOrders()`: 활성 POS 테이블 액티비티에서 주문 상태의 주문 목록을 반환한다.
- `getPendingOrders()`: 활성 POS 테이블 액티비티에서 승인 대기 상태의 주문 목록을 반환한다.
...
- `tableNo`: 테이블 번호
- `discount`: 할인 금액
- `pendingDiscount`: 승인 대기 주문의 할인 금액
- `active`: 활성 여부
...
- `hasPendingOrder()`: 액티비티에 승인 대기 상태의 주문이 있는지 여부를 반환한다.
...
- `getTableOrderType()`: 액티비티의 주문들을 통해 액티비티의 주문 타입을 반환한다.
- `getOrder()`: 액티비티에서 주문을 찾는다.
- `getOrderedOrder()`: 액티비티에서 주문 상태의 주문을 찾는다.
- `getOrderedOrPendingOrders()`: 액티비티에 승인 대기 상태의 주문이 있다면
  승인 대기 상태의 주문 목록을 반환하고 아니라면 주문 상태의 주문 목록을 반환한다.
- `getOrderedOrders()`: 액티비티에서 주문 상태의 주문 목록을 반환한다.
- `getPendingOrders()`: 액티비티에서 승인 대기 상태의 주문 목록을 반환한다.
- `getOrderedOrPendingTypePayments()`: 액티비티에 승인 대기 상태의 주문이 있다면
  승인 대기 상태의 주문에 대한 결제 목록을 반환하고 아니라면 승인 완료 상태의 주문에 대한 결제 목록을 반환한다.
- `getPendingTypePayments()`: 액티비티에서 승인 대기 상태의 주문에 대한 결제 목록을 반환한다.
- `getOrderedTypePayments()`: 액티비티에서 승인 완료 상태의 주문에 대한 결제 목록을 반환한다.
- `getDiscountPrice()`: 액티비티의 주문의 타입에 맞는 할인 금액을 반환한다.
