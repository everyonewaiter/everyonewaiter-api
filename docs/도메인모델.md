...
- `address`: 매장 주소
- `landline`: 매장 유선 전화번호
- `license`: `BusinessLicense` 사업자 등록 번호
...
- `account`: 매장 등록을 신청한 계정
- `detail`: `BusinessDetail` 매장 상세 정보
- `status`: `RegistrationStatus` 매장 등록 신청 상태
- `rejectReason`: 매장 등록 신청 반려 사유

#### 행위

- `static create()`: 매장 등록을 신청한다. // TODO: 도메인 용어에 맞게 메서드 명 변경 -> apply
...
- `printerLocation`: `PrinterLocation` 주방 프린터 연결 위치
- `showMenuPopup`: 손님 테이블 메뉴 클릭 시 팝업 출력 여부
- `showOrderTotalPrice`: 손님 테이블 주문 내역 총 주문 금액 출력 여부
- `countryOfOrigins`: `CountryOfOrigin` 음식 원산지 정보 목록
- `staffCallOptions`: `StaffCallOption` 직원 호출 옵션 목록
