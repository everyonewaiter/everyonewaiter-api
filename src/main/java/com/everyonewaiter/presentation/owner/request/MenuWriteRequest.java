...
public class MenuWriteRequest {
  ...
  public record Create(
      ...
      List<@Valid CreateOptionGroup> menuOptionGroups
  ) {
    ...
    public MenuWrite.Create toDomainDto(MultipartFile file) {
      return new MenuWrite.Create(
          ...
          menuOptionGroups.stream()
              .map(CreateOptionGroup::toDomainDto)
              .toList(),
          file
      );
    }
  ...
  @Schema(name = "MenuWriteRequest.CreateOptionGroup")
  public record CreateOptionGroup(
      ...
      @Size(max = 20, message = "메뉴 옵션은 옵션 그룹당 최대 20개까지 등록할 수 있어요.")
      List<@Valid CreateOption> menuOptions
  ) {

    public MenuWrite.CreateOptionGroup toDomainDto() {
      return new MenuWrite.CreateOptionGroup(
          name,
          type,
          printEnabled,
          menuOptions.stream()
              .map(CreateOption::toDomainDto)
              .toList()
      );
    }
  ...
  @Schema(name = "MenuWriteRequest.CreateOption")
  public record CreateOption(
    ...
    public MenuWrite.CreateOption toDomainDto() {
      return new MenuWrite.CreateOption(name, price);
    }
