...
class AccountSignInService implements AccountSignInHandler {
  ...
  @Override
  public SignInToken signIn(AccountSignInRequest signInRequest, AccountPermission permission) {
    Email email = new Email(signInRequest.email());

    Account account = accountFinder.find(email).orElseThrow(FailedSignInException::new);

    account.signIn(signInRequest, passwordEncoder, permission);
  ...
  private SignInToken createToken(Account account) {
